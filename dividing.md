...
而当组件逐渐复杂化，可以更具组件范围抽取出对应的以上3种 viewModel。
...
随着产品的调用发现针对用户的操作需要加些门槛，譬如：

1. 每分钟内用户只允许编辑几次
2. 每天用户只能新增几个用户
3. 等等...

这时候如果我们依旧把交互逻辑放在 react 组件内会发现该组件已经变得非常臃肿，后期维护很麻烦，而且流程控制是 saga 的强项，个这个时候我们可以将这块交互逻辑进行抽取形成一个 viewModel。
